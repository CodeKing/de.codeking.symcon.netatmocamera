...
class splNetatmoAPI
{
    ...
    public function setLightMode($camName, $mode = 'auto') //Presence
    {
        ...
        $value = null;
        if (in_array($mode, ['on', '1', 1, true], true)) $value = 'on';
        if (in_array($mode, ['off', '0', 0, false], true)) $value = 'off';
        if (in_array($mode, ['auto', 2], true)) $value = 'auto';
        if (!isset($value)) return ['error' => 'Unsupported value'];
    ...
    public function setLightIntensity($camName, $intensity = 100) //Presence
    {
        ...
        if ($camera['type'] != 'Presence') return ['result' => null, 'error' => 'Unsupported camera for setLightIntensity()'];

        $intensity = intval($intensity);
        if ($intensity > 100 or $intensity < 0) return ['result' => null, 'error' => 'Presence Light use intensity range from 0 to 100'];
    ...
    protected function getCamByName($name) //Presence - Welcome
    {
        ...
        return ['result' => null, 'error' => 'Unfound camera'];
    }
    ...
    {
        if (empty($this->_persons)) return ['result' => null, 'error' => 'No person defined in this home.'];

        foreach ($this->_persons as $thisPerson) {
            if ($thisPerson['pseudo'] == $name) return $thisPerson;
        }
        return ['result' => null, 'error' => 'Unfound person'];
    }
